...
\author{%
\IEEEauthorblockN{\textless Your Name\textgreater}
\IEEEauthorblockA{\textless Course / Department\textgreater\\
\textless University\textgreater\\
...
The implemented project addresses that systems problem with \bertopic as the main model. The choice of \bertopic is central to the architecture because it combines contextual sentence embeddings, manifold reduction, density-based clustering, and class-based TF--IDF topic representation. This gives the system a strong semantic foundation for short and noisy text compared with purely lexical models. However, the project does not present \bertopic in isolation. It also includes \lda and \nmf as explicit benchmark models, allowing the quality of semantically derived topics to be compared against classical bag-of-words baselines.

From a final-project perspective, the most important contribution of the implementation is not a novel learning algorithm. Rather, it is the construction of an end-to-end topic monitoring system that operationalizes \bertopic across data ingestion, recurring model updates, drift assessment, interactive review, and dashboard presentation. The implementation therefore has both modeling and systems dimensions. On the modeling side, it uses \bertopic as the production topic model and benchmarks it against \lda and \nmf. On the systems side, it organizes the workflow into a recurring pipeline, exposes analytical outputs through an application programming interface, and provides a front-end for monitoring and human intervention.

A second important point is methodological honesty. This report is written only from what can be supported by the uploaded code and files. If a capability is partially implemented, that is stated explicitly. If a result comes from an external screenshot rather than a reproducible run within the uploaded archive, that is also stated explicitly. The goal is not to maximize claims, but to provide a technically strong and evidence-based explanation of what the project actually does.

\section{Project Objective and Implemented Scope}
The implemented system is designed around a clear separation between a \emph{primary operational model} and \emph{comparison models}. The primary operational model is \bertopic. It is the model used for topic discovery on incoming batches, topic metadata generation, user-facing topic inspection, and inference on unseen text. The comparison models are \lda and \nmf. Their role is evaluative rather than operational: they provide reference metrics so that the behavior of the semantic topic model can be contrasted with two classical alternatives.

This distinction is important because it clarifies the meaning of the project's ``three models.'' In this implementation, the phrase does not refer to three different saved \bertopic checkpoints or three abstract conceptual layers. It refers specifically to three topic-modeling approaches: \bertopic, \lda, and \nmf. Among these, only \bertopic serves as the live model around which the rest of the monitoring system is organized.

The implemented scope of the system includes the following capabilities.
\begin{enumerate}[leftmargin=*,itemsep=2pt]
    \item Recurring ingestion of dataset-specific text batches from a configured data source.
    \item Preprocessing that adapts to the active dataset profile.
    \item Topic modeling with \bertopic as the main model.
    \item Comparative evaluation with \lda and \nmf.
    \item Historical monitoring through batch identifiers and persistent outputs.
    \item Drift detection between current and prior topic model states.
    \item Interactive inspection through a dashboard.
    \item Human-in-the-loop relabeling and topic merge operations.
    \item Model versioning and rollback support.
\end{enumerate}

The implementation therefore goes beyond a standard classroom demonstration of topic modeling. At the same time, it is not a fully generalized production platform. Some features remain partial, some data dependencies are external to the uploaded archive, and some parts of the codebase still reflect an earlier single-dataset orientation. These limitations are discussed later rather than hidden.
